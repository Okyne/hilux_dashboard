...
Publie : tire_fl, tire_fr, tire_rl, tire_rr (bar).
...
class TpmsReader(threading.Thread):
    ...
    def _parse_line(line):
        ...
        Retourne (sensor_id, pression_bar) ou None. À ADAPTER à ton kit.
        """
        try:
            fields = dict(p.split("=") for p in line.split(";") if "=" in p)
            return fields["ID"], float(fields["P"])
        except Exception:
            return None
